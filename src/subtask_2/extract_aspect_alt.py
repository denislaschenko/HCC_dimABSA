...
def process_jsonl(model_name, input_path, output_path):
    ...
    with open(input_path, "r", encoding="utf-8") as fin, \
        ...
        for line in tqdm(fin, total=total_lines, desc="Extracting"):
            ...
            # --- MODIFICATION: Create parallel lists instead of Triplets ---
            aspects = []
            opinions = []
            ...
            new_entry = {
                "ID": item["ID"],
                "Text": text,
                "Aspect": aspects,   # Requested list format
                "Opinion": opinions  # Kept so we can reconstruct the pair later
            }
            # ---------------------------------------------------------------
